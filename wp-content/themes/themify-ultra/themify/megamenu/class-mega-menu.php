	...
	class Themify_Mega_Menu_Walker extends Walker_Nav_Menu {
		...
		function render_widget_menu( $item, $depth, $class_names = '' ) {
			...
			if( $depth === 0 ) {
				...
				$output = "<li id='menu-item-$item->ID' $class_names><a href='#'>" . $title . '<span class="child-arrow"></a></span><ul class="sub-menu tf_box">';
			}
		...
		function render_layout_part( $item, $depth, $class_names = '' ) {
			...
			if( $depth === 0 ) {
				$title = apply_filters( 'the_title', $item->title, $item->ID );
				$output = "<li id='menu-item-$item->ID' $class_names><a href='#'>" . $title . '<span class="child-arrow"></span></a><ul class="sub-menu tf_box">';
			}
