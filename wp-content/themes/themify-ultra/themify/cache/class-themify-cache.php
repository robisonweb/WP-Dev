...
class TFCache {
    ...
    public static function hooks() {
		...
		add_action('admin_notices',array(__CLASS__,'cache_plugins'),10);
		if(isset($_GET['tf_cache']) && $_GET['tf_cache']==='2' && is_user_logged_in()){
    ...
    public static function run(){
		...
		if(is_dir(self::$cache_dir) || mkdir(self::$cache_dir)){
			...
			if($isMulti===false || is_dir(self::$cache_dir) || mkdir(self::$cache_dir)){
				...
				add_action('get_header',array(__CLASS__,'tf_cache_start'),0);
			}
			else{
				self::$cache_dir=null;
			}
    ...
    public static function tf_cache_start(){
		...
		ob_start();
		add_action('wp_footer',array(__CLASS__,'tf_body_end'));
    }
	...
	public static function cache_menu($wp_admin_bar){
		...
		$link=remove_query_arg('tf_cache',self::get_current_url());
		$args = array(
			array(
				'id'=>'tf_clear_cache',
		...
		$args[]=array(
			...
			'href'=>add_query_arg(array('tf_cache'=>1),$link),
			'title'=>$hasCache===true?__('Purge Page Cache','themify'):__('Regenerate Page CSS','themify')
		);
		$args[]=array(
			'id'=>'tf_clear_all',
			'parent'=>'tf_clear_cache',
			'href'=>add_query_arg(array('tf_cache'=>2),$link),
		...
		if($hasCache===false && $cache_plugins==false){
			...
			$args[]=array(
				'id'=>'tf_clear_menu',
				'parent'=>'tf_clear_cache',
				'href'=>add_query_arg(array('tf_cache'=>3),$link),
		...
		if(!empty($_GET['tf_cache'])){
			$cache_type=(int)$_GET['tf_cache'];
	...
	public static function cache_plugins(){
		...
		if(!empty($cache_plugins)){
			self::disable_cache();
		?>
			<div class="notice notice-error tf_compatible_erros tf_erros">
				<p><strong><?php _e('Themify Cache can not be enabled due to another cache plugin is activated.','themify');?></strong></p>
			</div>
		<?php
		}
	}
