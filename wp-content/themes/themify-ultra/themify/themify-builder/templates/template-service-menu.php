...
if ( !empty( $fields_args['image_service_menu'] ) ) {
	if ( Themify_Builder_Model::is_img_php_disabled() ) {
		...
		$attachment_id = themify_get_attachment_id_from_url( $fields_args['image_service_menu']);
		$class = $attachment_id ? 'wp-image-' . $attachment_id : '';
		$image = $fields_args['image_service_menu'] ? '<img src="' . themify_generateWebp(esc_url( $fields_args['image_service_menu'] )) . '" alt="' . esc_attr( $image_alt ) . '" width="' . $width_service_menu . '" height="' . $height_service_menu . '" class="tb_menu_image ' . $class . '">': '';
	} else {
		$image = themify_get_image( array('src'=>esc_url( $fields_args['image_service_menu'] ) , 'w'=> $fields_args['width_service_menu'], 'h'=>$fields_args['height_service_menu'],'alt'=> $image_alt,'class'=>'tb_menu_image' ));
	}
}
