	...
	var w=data.circle_stroke_feature || 0,
		...
		<span class="module-feature-chart-html5 tf_box tf_rel tf_inline_b"<# if(st!==''){#> style="{{st}}"<#}#>>
			<# if(!isEmpty){#>
				...
				<circle class="tb_feature_fill" style="{{r}}" cx="50%" cy="50%" stroke-width="{{w}}"/>
				<circle class="tb_feature_stroke" style="{{r}}" cx="50%" cy="50%" stroke="{{tb_app.Utils.toRGBA(data.circle_color_feature)}}" stroke-width="{{w}}" data-progress="{{data.circle_percentage_feature}}" stroke-dasharray="0,10000"/>
				<# if(insetColor !== '' && data.icon_type_feature=='icon'){#>
				    <circle class="tb_feature_bg" style="r:calc(50% - <# print((w>1?(w-1):0))#>px)" cx="50%" cy="50%" stroke-width="{{w}}" fill="{{insetColor}}" />
				<# }#>
			    </svg>
			<# }#>
