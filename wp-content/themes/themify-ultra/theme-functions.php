...
function themify_theme_enqueue_footer() {
	...
	if ( $font !== 'default' ) {
		...
		themify_enque_style( 'ultra-font-'.$font, THEME_URI . '/styles/fonts/' . $font . '.min.css', null, $theme_version );
	}
	if ($color !== 'default' ) {
		$color=str_replace('theme-color-','',$color);
		themify_enque_style( 'ultra-color-'.$color, THEME_URI . '/styles/colors/' . $color . '.css', null, $theme_version );	}
	...
	if(themify_theme_is_single_infinite_enabled()){
		Themify_Enqueue_Assets::addPrefetchJs(THEME_URI . '/js/modules/single-infinite.js',$theme_version);
...
function themify_mobile_menu_expand_toggle(){
	if(themify_theme_show_area( 'menu_navigation' ) && themify_theme_do_not_exclude_all( 'mobile-menu' )){
		if(themify_check( 'setting-mobile_menu_expand',true )){
			add_filter( 'nav_menu_css_class', 'themify_mobile_menu_expand', 10, 2 );
		}
...
/**
 * Auto expand mobile menus
 */
function themify_mobile_menu_expand( $classes, $item ){
	if($item->menu_item_parent==0){
		$classes[] = 'toggle-on';
	}
	return $classes;
...
/**
 * Make the mobile menu accordion
 */
